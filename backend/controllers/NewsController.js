...
const uploadNewsImage = async (imageFile) => {
    if (!imageFile) return null;
    const fileName = `${awsFolderNames.news}/${Date.now()}-${imageFile.name}`;
...
const createNews = async (req, res) => {
    try {
        ...
        const commonImage = await uploadNewsImage(req.files?.commonImage);
        const imageEn = await uploadNewsImage(req.files?.imageEn);
        const imageSi = await uploadNewsImage(req.files?.imageSi);
        const imageTa = await uploadNewsImage(req.files?.imageTa);
    ...
    try {
        ...
        if (req.files) {
            if (req.files.commonImage) {
                await deleteImageFromAwsIfExist(news.commonImage);
                news.commonImage = await uploadNewsImage(req.files.commonImage);
            }
            if (req.files.imageEn) {
                await deleteImageFromAwsIfExist(news.imageEn);
                news.imageEn = await uploadNewsImage(req.files.imageEn);
            }
            if (req.files.imageSi) {
                await deleteImageFromAwsIfExist(news.imageSi);
                news.imageSi = await uploadNewsImage(req.files.imageSi);
            }
            if (req.files.imageTa) {
                await deleteImageFromAwsIfExist(news.imageTa);
                news.imageTa = await uploadNewsImage(req.files.imageTa);
            }
        }
