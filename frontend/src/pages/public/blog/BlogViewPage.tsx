...
const BlogViewScreen = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await blogApi.getBlogById(id as string) as any;
                if (response.success) {
                    setBlogItem(response.data);
                } else {
                    setError(response.message || "Failed to fetch blog item. Please try again.");
                }
            } catch (err) {
    ...
    const getLocalizedField = (fieldName: string) => {
        if (currentLang.startsWith('si')) {
            const val = blogItem[`${fieldName}Si`];
            return isFieldEmpty(val) ? blogItem[`${fieldName}En`] : val;
        }
        if (currentLang.startsWith('ta')) {
            const val = blogItem[`${fieldName}Ta`];
            return isFieldEmpty(val) ? blogItem[`${fieldName}En`] : val;
        }
        return blogItem[`${fieldName}En`];
    };

    const title = getLocalizedField('title') || "No title available";
    const content = getLocalizedField('content') || "No content available";
    const image = (currentLang.startsWith('si') && blogItem.imageSi) ? blogItem.imageSi :
                  (currentLang.startsWith('ta') && blogItem.imageTa) ? blogItem.imageTa :
                  (currentLang.startsWith('en') && blogItem.imageEn) ? blogItem.imageEn :
                  blogItem.commonImage;

    return (
        <section id="updates-blog" className="bg-gray-50 py-12">
            <div className="max-w-6xl mx-auto px-4">
                <Link to="/all-blog" className="inline-flex items-center text-main-color hover:text-third-color mb-6">
                ...
                <article className="bg-white rounded-lg shadow-md overflow-hidden">
                    ...
                    <div className="p-4 md:p-6 lg:p-8">
                        ...
                        <p className="text-sm text-third-color mb-6">
                            {blogItem.date || "No date available"}
                        </p>
