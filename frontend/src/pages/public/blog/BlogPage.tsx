...
const AllBlogScreen = () => {
  ...
  // Filter blog based on search query
  ...
  }, [blogList, searchQuery, i18n.language]);
    ...
    return (
      <div className="min-h-screen bg-canvas dark:bg-dark-bg flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-text-secondary dark:text-gray-400">Loading blog...</p>
        </div>
      </div>
    );
  }
      ...
      <section className="bg-gradient-to-br from-[#5D1840] via-[#4C1333] to-[#360C23] py-16">
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          <p className="text-lg text-white/80 mb-8 font-body">
            Stay informed with the latest blog and updates from NIRDC
          </p>
        ...
        <div className="max-w-6xl mx-auto px-4">
              ...
              <div className="p-4 bg-primary/10 dark:bg-primary-light/10 rounded-xl border border-primary/20 dark:border-primary-light/30 flex-1 mr-4">
                <p className="text-primary dark:text-primary-light font-medium">
                  {filteredBlog.length > 0 
                    ? `Found ${filteredBlog.length} result${filteredBlog.length !== 1 ? 's' : ''} for "${searchQuery}"`
          ...
          {blogList.length === 0 && !isLoading ? (
            <div className="text-center py-12">
              <p className="text-text-secondary dark:text-gray-400 text-lg">No blog articles available at the moment.</p>
              <p className="text-text-secondary dark:text-gray-500 text-sm mt-2">Please check back later for updates.</p>
            </div>
          ) : filteredBlog.length === 0 && searchQuery ? (
            <div className="text-center py-12">
              <div className="mb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredBlog.map((item: any) => {
                ...
                const title = getLocalizedField('title') || "Untitled Article";
                const content = getLocalizedField('content') || "Content not available at the moment.";
