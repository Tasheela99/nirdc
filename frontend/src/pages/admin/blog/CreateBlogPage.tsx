...
const LOCAL_STORAGE_KEY = "blogFormData";
const EXPIRATION_KEY = "blogFormDataExpiration";
...
const CreateBlogScreen = () => {
    ...
    const editBlog = state?.blog as EditBlog;
    ...
    const [formData, setFormData] = useState<BlogFormData>(() => {
        ...
        if (editBlog) {
            return {
                titleEn: editBlog.titleEn || "",
                titleSi: editBlog.titleSi || "",
                titleTa: editBlog.titleTa || "",
                descriptionEn: editBlog.descriptionEn || "",
                descriptionSi: editBlog.descriptionSi || "",
                descriptionTa: editBlog.descriptionTa || "",
                commonImage: null,
                imageEn: null,
                imageSi: null,
                imageTa: null,
                date: editBlog.date ? editBlog.date.substring(0, 10) : "",
            };
        }
    ...
    useEffect(() => {
        if (editBlog) {
            setImagePreviewUrls({
                commonImage: editBlog.commonImage || null,
                imageEn: editBlog.imageEn || null,
                imageSi: editBlog.imageSi || null,
                imageTa: editBlog.imageTa || null
            });
        }
    }, [editBlog]);
    ...
    useEffect(() => {
        if (editBlog) return; // Don't auto-save in edit mode
    ...
    useEffect(() => {
        return () => {
            ...
            ['commonImage', 'imageEn', 'imageSi', 'imageTa'].forEach(key => {
                if (imagePreviewUrls[key] && !(editBlog as any)?.[key]) {
                    URL.revokeObjectURL(imagePreviewUrls[key] as string);
                }
            });
        };
    }, [imagePreviewUrls, editBlog]);
    ...
    const handleChange = (
        ...
        if (type === "file") {
            ...
            if (file) {
                ...
                if (imagePreviewUrls[name] && !(editBlog as any)?.[name]) {
    ...
    const validateForm = (): boolean => {
        ...
        if (!editBlog && (!formData.commonImage || formData.commonImage.length === 0)) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (editBlog) {
                ...
                response = await blogApi.updateBlog(editBlog._id, formDataToSend) as ApiResponse;
            } else {
                // Create new blog
                response = await blogApi.createBlog(formDataToSend) as ApiResponse;
            }

            if (response?.status) {
                showAlert(
                    editBlog ? "Blog updated successfully!" : "Blog created successfully!",
                    "success"
                );
                ...
                // Navigate back to manage blog
                navigate("/admin/manage-blog");
            } else {
                showAlert(
                    response?.message || `Failed to ${editBlog ? "update" : "create"} blog article.`,
                    "error"
                );
            }
        } catch (error: any) {
            showAlert(
                ...
                `An error occurred while ${editBlog ? "updating" : "creating"} the blog article.`,
                "error"
            );
        } finally {
    ...
    const hasImage = formData.commonImage || formData.imageEn || formData.imageSi || formData.imageTa || (editBlog && editBlog.commonImage);
            ...
            >
                <Box display="flex" alignItems="center" justifyContent="space-between" position="relative" zIndex={1}>
                    <Box display="flex" alignItems="center" gap={3}>
                        ...
                        >
                            <ArticleIcon sx={{ color: 'white', fontSize: 36 }} />
                        </Avatar>
                        <Box>
                            <Typography variant="h3" color="white" fontWeight="700" sx={{ mb: 1 }}>
                                {editBlog ? 'Edit Blog Article' : 'Create Blog Article'}
                            </Typography>
                            <Typography variant="h6" color="rgba(255,255,255,0.8)" fontWeight="400">
                                {editBlog 
                                    ? 'Update and modify your blog article content' 
                                    : 'Craft and publish professional blog articles to your audience'
                                }
                            </Typography>
                        ...
                        <CardHeader
                            ...
                            subheader={
                                <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
                                    Fill in the information below to create your professional blog article
                                </Typography>
                            }
                            action={
                                <LinearProgress 
                            ...
                            <form onSubmit={handleSubmit}>
                                ...
                                >
                                    ...
                                    <TextField
                                        ...
                                        placeholder={`Enter blog article title in ${activeTab === 0 ? 'English' : activeTab === 1 ? 'Sinhala' : 'Tamil'}...`}
                                ...
                                >
                                    ...
                                    >
                                        <RichTextEditor
                                            ...
                                            placeholder={`Write your blog article content in ${activeTab === 0 ? 'English' : activeTab === 1 ? 'Sinhala' : 'Tamil'}...`}
                                            height={200}
                                        />
                                    </Box>
                                </Paper>
                                    ...
                                    <Grid item xs={12} md={6}>
                                        ...
                                        >
                                            <Box display="flex" alignItems="center" gap={2} mb={3}>
                                                ...
                                                <Box flex={1}>
                                                    <Typography variant="h6" fontWeight="700" color="text.primary">
                                                        Common Featured Image {!editBlog && <span style={{ color: 'red' }}>*</span>}
                                                    </Typography>
                                                    <Typography variant="body2" color="text.secondary">
                ...
                <Grid item xs={12} lg={4}>
                    ...
                    >
                        <CardHeader
                            ...
                            subheader={
                                <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
                                    See how your blog article will look
                                </Typography>
                            }
                            action={
                                <Stack direction="row" spacing={1}>
                        ...
                        <CardContent sx={{ p: 0 }}>
                            ...
                            <Box sx={{ p: 3 }}>
                                ...
                                >
                                    {activeTitle || 'Blog Article Title'}
                                </Typography>
